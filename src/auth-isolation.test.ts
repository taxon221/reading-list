...
describe("auth and user isolation", () => {
  ...
  test("keeps items, tags, and highlights isolated per user", async () => {
    ...
    const adminItems = await apiJson<ItemResponse[]>("/api/items", {}, bootstrapAdminEmail);
    ...
    const secondUserTags = await apiJson<Array<{ name: string; count: number }>>(
      "/api/tags",
      {},
      secondUserEmail,
    );
    ...
    const blockedRead = await api(`/api/items/${adminCreate.id}`, {}, secondUserEmail);
  ...
  test("serves uploaded files only to the owning user", async () => {
    ...
    expect(ownerResponse.headers.get("content-type")).toContain("application/pdf");
  ...
  test("migrates legacy single-user data to the bootstrap admin", async () => {
    ...
    const migratedItem = db
      ...
      .get() as { user_id: number; title: string; author: string; notes: string };
