...
export function setupTagInput(input, tagArray, container, options = {}) {
	...
	const getMatchingSuggestions = () => {
		...
		const source = Array.isArray(state.availableTags)
			? state.availableTags
			: [];
		if (!query) return [];
...
export function applySearch(items, query, tokens = []) {
	...
	const freeTerms =
		trimmed &&
		(!draftToken || draftToken.kind === "text") &&
		!looksLikeStructuredSearchDraft(trimmed)
...
export function parseReadingProgress(raw) {
	...
	return typeof value === "object" || typeof value === "number" ? value : null;
}
