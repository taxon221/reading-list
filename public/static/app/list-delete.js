...
function getDeleteChoices() {
	if (deleteBy === "tag")
		return (deleteFacetsData.tags || []).map((tag) => tag.name);
...
function renderDeleteByOptions() {
	dom.deleteByOptions
		?.querySelectorAll("[data-delete-by]")
		.forEach((option) => {
			option.classList.toggle(
				"is-active",
				option.dataset.deleteBy === deleteBy,
			);
		});
}
...
function updateDeleteValueUi() {
	...
	if (deleteBy === "domain")
		dom.deleteValueTextInput.placeholder = "Search websites...";
	else if (deleteBy === "author")
		dom.deleteValueTextInput.placeholder = "Search authors...";
	else if (deleteBy === "tag")
		dom.deleteValueTextInput.placeholder = "Search tags...";
...
async function confirmDeleteItems() {
	...
	if (
		!confirm(
			`Delete all items where ${by} matches ${preview}? This cannot be undone.`,
		)
	)
		return;
	...
	closeDropdowns();
	alert(
		data.deleted ? `Deleted ${data.deleted} item(s).` : "No matching items.",
	);
	refreshItems();
	refreshTags();
}
...
export function initListDelete({
	...
	closeDropdowns =
		typeof closeAllDropdowns === "function" ? closeAllDropdowns : () => {};
	...
	dom.deleteValueOptions?.addEventListener("change", (event) => {
		...
		if (input.checked) {
			...
			deleteSelectedValues = deleteSelectedValues.filter(
				(item) => item !== value,
			);
		}
		renderDeleteValueOptions();
