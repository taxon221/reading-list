...
function updateThemeColorMeta(resolvedMode) {
  ...
  themeColor.setAttribute("content", resolvedMode === "dark" ? "#111111" : "#eef1f4");
}
...
function cycleThemeMode() {
  ...
  const currentMode = normalizeThemeMode(localStorage.getItem(THEME_STORAGE_KEY));
...
export function initTheme() {
  ...
  const handleSystemThemeChange = () => {
    if (normalizeThemeMode(localStorage.getItem(THEME_STORAGE_KEY)) !== "system") return;
    applyTheme("system");
  };
